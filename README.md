...
**Grabación programada — `O → T → G → N`.** Elige una favorita, hora `HHMM` y duración de 1 a 1440 minutos. `P` alterna **Parar reproducción al finalizar: Sí/No** (No por defecto). Revisa emisora, fecha de inicio/fin y parada; `Enter` confirma, `?` permite leer el detalle completo y `Esc` conserva la programación anterior. Si la hora ya pasó se propone mañana. En `O → T → G` puedes consultar el estado o cancelar/detener con `X` y confirmación; nunca borra el archivo.

Una programación por sesión, con **Keila abierto y el equipo despierto**: no despierta Android ni se conserva al cerrar. Al inicio puede cambiar la radio; respeta volumen y silencio, espera hasta 30 s a recibir audio y muestra «Preparando archivo», «Grabando» y «Cierre pendiente». El fin previsto no se retrasa. Deja la emisora sonando salvo que actives la parada final; no vuelve a la anterior. Guarda en la carpeta habitual con nombre exclusivo y verificación del cierre.

Con la parada final activa, primero cierra el archivo y después detiene la radio y los reintentos, incluso si el archivo requiere revisión. **No apaga el equipo ni cierra Keila.** Solo actúa sobre su propia reproducción al llegar al final previsto, no al cancelar manualmente ni si se omitió la grabación. Conserva alarmas futuras; una alarma ya vencida no reactiva la radio.

**Grabar mientras duermes:** silencia con `M`, no con pausa. Tanto la grabación manual como la programada guardan el audio original de la emisora: el volumen de escucha, incluso a cero, y el silencio no atenúan el archivo. No hace falta un volumen de grabación separado; al escucharlo después ajustas el volumen del reproductor. Si Keila sigue silenciado, quita `M` para oírlo.
