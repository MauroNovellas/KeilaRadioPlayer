...
preferences_build_rows() {
    ...
    if [[ "$mode" == settings ]]; then
        ...
        panel_add_row '' 'Grabación programada' 'Opciones > Temporizador > G > N: favorita, HHMM y duración (1–1440 min). Revisa fecha, emisora y fin antes de confirmar; ? muestra todo el detalle. X cancela o cierra su grabación tras confirmar. Una reserva por sesión; requiere Keila abierto y equipo despierto. Respeta volumen/silencio; puede cambiar la radio, no restaura la anterior. Omite inicios más de 60 s tarde y espera hasta 30 s de audio. No interrumpe otra grabación; la parada automática tiene prioridad.'
