...
ui_search_desktop() {
    ...
    local row match_position source_index result_text result_badge result_style selected
    ...
    for ((row = 0; row < body_height; row++)); do
        ...
        ui_desktop_row "$left_text" "$left_badge" "$left_style" "$left_badge_style" \
            "$result_text" "$result_badge" "$result_style" "$result_style" "$selected"
    done

    ui_desktop_join_rule "$width"
    ui_box_line "$width" "Escribe para filtrar  $UI_SEP  ↑↓ mover  $UI_SEP  Enter reproducir  $UI_SEP  Backspace borrar  $UI_SEP  Esc volver" muted
...
ui_search_single_column() {
    ...
    local row match_position source_index text badge selected style
    for ((row = 0; row < body_height; row++)); do
        match_position=$((SEARCH_SCROLL_OFFSET + row))
        if ((match_position >= ${#SEARCH_MATCHES[@]})); then
        ...
        player_is_running && [[ "${SEARCH_URLS[$source_index]}" == "$PLAYER_URL" ]] && badge='[PLAY]'
        ui_box_split_line "$width" "$text" "$badge" "$selected" "$style" "$style"
    done

    ui_box_rule "$width" "$UI_ML" "$UI_MR"
    ui_box_line "$width" "Escribe  $UI_SEP  ↑↓  $UI_SEP  Enter play  $UI_SEP  Backspace  $UI_SEP  Esc" muted
    if [[ -n "$UI_MESSAGE" ]]; then
        ui_box_line "$width" "$UI_MESSAGE"
    else
        ui_box_line "$width" ''
...
ui_draw_search() {
    ...
    if [[ "$UI_LAYOUT_MODE" == 'tiny' ]]; then
        ...
        printf '\n\n'
        ui_print_padded "$tiny_width" 'Esc = volver'
        printf '\n'
        tput ed 2>/dev/null || true
        return 0
    fi
